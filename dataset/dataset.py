...
# Define the dataset (from the examples in the PS)
data = [
    {
    ...
    },
    {
        ...
        "json_output": [
            {"tool_name": "whoami", "arguments": []},
            {
                "tool_name": "works_list",
                "arguments": [
                    {"argument_name": "stage.name", "argument_value": ["triage"]},
        ...
        "json_output": [
            {
                "tool_name": "search_object_by_name",
                "arguments": [
                    {"argument_name": "query", "argument_value": "Cust123"}
                ]
            },
            {
                "tool_name": "works_list",
        ...
        "json_output": [
            {
                ...
                "arguments": [
                    {"argument_name": "text", "argument_value": "T"}
                ]
            },
            {"tool_name": "get_sprint_id", "arguments": []},
            {
        ...
        "json_output": [
            {
                "tool_name": "get_similar_work_items",
                "arguments": [
                    {"argument_name": "work_id", "argument_value": "TKT-123"}
                ]
            },
            {
                "tool_name": "summarize_objects",
            ...
            },
            {
                "tool_name": "create_actionable_tasks_from_text",
                "arguments": [
                    {"argument_name": "text", "argument_value": "$$PREV[1]"}
                ]
            },
            {
                "tool_name": "prioritize_objects",
                "arguments": [
                    {"argument_name": "objects", "argument_value": "$$PREV[2]"}
                ]
            }
        ]
    }
...
# Convert JSON outputs to strings for CSV
for item in data:
    item["json_output"] = json.dumps(item["json_output"], indent=2)

# Create DataFrame and save
df = pd.DataFrame(data)
df.to_csv("dataset.csv", index=False)
print("✅ Dataset created successfully as dataset.csv")

with open("dataset.jsonl", "w") as f:
    for item in data:
        json.dump(item, f)
        f.write("\n")
print("✅ dataset.jsonl also created")